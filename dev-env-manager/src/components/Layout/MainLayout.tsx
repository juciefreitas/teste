...
import { AppShell, Text, Group, Button, useMantineTheme } from '@mantine/core';
...
const MainLayout: React.FC<MainLayoutProps> = ({ children, consoleSlot }) => {
  ...
  return (
    <AppShell
      padding="md"
      header={{ height: 60 }}
      footer={consoleSlot ? { height: currentConsoleHeight } : undefined}
      styles={(th) => ({
        main: {
    ...
    >
      ...
      {consoleSlot && (
        <AppShell.Footer p={0} style={{ borderTop: `1px solid ${theme.colors.gray[3]}` }}>
